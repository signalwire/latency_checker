...
@click.option('--onset-rel-frac', default=0.10, type=float,
              help='Segment onset must reach this fraction of the segment peak '
                   '(rejects pre-speech transients; 0 disables)')
...
def analyze_audio(audio_file, threshold, ai_min_speaking, human_min_speaking, min_silence, onset_peak_mult, onset_rel_frac, output, format, sample_rate, crosstalk_ratio, quiet):
    """
    ...
    try:
        if not quiet:
            ...
            click.echo(f"Settings: threshold={threshold}, ai_min={ai_min_speaking}ms, human_min={human_min_speaking}ms, silence={min_silence}ms, onset_peak_mult={onset_peak_mult}, onset_rel_frac={onset_rel_frac}")
            click.echo("-" * 60)
